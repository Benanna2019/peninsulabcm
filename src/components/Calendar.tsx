...
// Comeback and add zod schema validation on these events.
// Need to add it on inital fetch from sanity so I know I am getting the right information
...
export default function Calendar({ calendarEvents }: any) {
  ...
  if (
    ...
    data.allCalendarEvents.length === 0 ||
    isError
  ) {
    return (
      <NoDataLayout
        no_data_heading={"No Calendar Events Yet"}
        no_data_info={"Check back in the future for events on the calendar"}
      >
              ...
              {days.map((day, dayIdx) => (
                ...
                >
                  ...
                  <div className="w-1 h-1 mx-auto mt-1">
                    {allCalendarEvents.some((event: any) =>
                      isSameDay(parseJSON(event.startDate), day)
                    ) && (
          ...
          <section className="mt-12 md:mt-0 md:pl-14">
            ...
            <ol className="mt-4 space-y-1 text-sm leading-6 text-gray-500">
              {selectedDayMeetings.length > 0 ? (
                selectedDayMeetings.map((event: any) => (
    ...
    <li className="flex items-center px-4 py-2 space-x-4 group rounded-xl focus-within:bg-gray-100 hover:bg-gray-100">
      ...
      <div className="flex-auto">
        ...
        <p className="mt-0.5">
          ...
          -{" "}
          <time dateTime={event?.endDate}>{format(endDateTime, "h:mm a")}</time>
        </p>
      </div>
      <Menu
        as="div"
